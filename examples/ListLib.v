...
(* STAR *)
Definition skipn' {A: Type} (pos : nat) (l : list A): list A :=
  sublist (pos) (length l) l.
...
(* STAR： 嵌套 *)
Lemma sublist_sublist {A : Type} (lo hi lo' hi' : nat) (l : list A): 
  (lo <= hi /\ lo' <= hi') ->
  (lo + hi' <= hi) ->
  sublist lo' hi' (sublist lo hi l) = sublist (lo + lo') (lo + hi') l.
Proof.
  intros.
  unfold sublist.
  repeat rewrite skipn_firstn.
  ...
  rewrite skipn_skipn.
  set (l1 := skipn (lo' + lo) l).
  replace (skipn (lo + lo') l) with l1.
  2:{ rewrite Nat.add_comm. easy. }
  destruct (Nat.lt_trichotomy (hi' - lo') (hi - lo - lo')) as [Hlt | [Heq | Hgt]];
  replace (Init.Nat.min (hi' - lo') (hi - lo - lo')) with (hi' - lo') by lia;
  replace (lo + hi' - (lo + lo')) with (hi' - lo') by lia;
  easy.
Qed.

(* STAR: sublist 扩展相等，改写为了无app的版本 *)
Lemma sublist_hi_plus_one {A : Type} (default : A) (lo hi lo' hi' : nat) (l1 l2: list A):
  (lo <= hi) ->
  (hi + 1 <= length l1) ->
  (lo' <= hi') ->
  (hi' + 1 <= length l2) ->
  (sublist lo hi l1 = sublist lo' hi' l2) ->
  (nth hi l1 default = nth hi' l2 default) ->
  (sublist lo (hi + 1) l1 = sublist lo' (hi' + 1) l2).
Proof.
  intros.
  rewrite (sublist_split lo (hi + 1) hi); try lia.
  rewrite (sublist_split lo' (hi' + 1) hi'); try lia.
  rewrite H3.
  rewrite (sublist_single hi l1 default); try lia.
  rewrite (sublist_single hi' l2 default); try lia.
  rewrite H4.
  easy.
Qed.
...
(* STAR *)
Definition is_proper_prefix {A : Type} (l1 l2 : list A) : Prop :=
  is_prefix l1 l2 /\ length l1 < length l2.
...
(* STAR *)
Lemma sublist_is_prefix {A : Type} (lo hi hi' : nat) (l : list A):
  (lo <= hi <= hi') ->
  (hi' <= length l) ->
  is_prefix (sublist lo hi l) (sublist lo hi' l).
Proof.
  intros.
  unfold is_prefix.
  exists (sublist hi hi' l).
  rewrite (sublist_split lo hi' hi l); try lia.
  easy.
Qed.

(* STAR *)
Lemma sublist_is_suffix {A : Type} (lo lo' hi: nat) (l : list A):
  (lo' <= lo <= hi) ->
  (hi <= length l) ->
  is_suffix (sublist lo hi l) (sublist lo' hi l).
Proof.
  intros.
  unfold is_suffix.
  exists (sublist lo' lo l).
  rewrite (sublist_split lo' hi lo l); try lia.
  easy.
Qed.

(* STAR *)
Lemma is_prefix_self:
  forall {A : Type} (l : list A), is_prefix l l.
Proof.
  intros.
  unfold is_prefix.
  exists nil.
