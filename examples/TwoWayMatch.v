...
(* Connect the preprocessing phase with this matching phase *)

(* MARK: Using Znth clarifies index range validity *)
Definition local_period (l w: Z): Prop :=
  0 <= l <= Zlength patn /\ 0 < w /\ (* TODO: replace <= with < here *)
  forall i,
    0 <= i < w -> (* Default index range *)
    0 <= l + i - w < l -> (* Index validity *)
    l + i < Zlength patn -> 
  ...
  - unfold local_period.
    ...
    + assert (Zlength patn = 0 \/ 0 < Zlength patn) by lia.
      ...
      * apply Zlength_zero_iff_nil in H0.
        contradiction.
      * lia.
    + intros. lia. (* Trivial: no legal range *)
Qed.

(* nat version of “a nonempty set has a minimum”, without decidability (Classical) *)
...
Proof.
  intros l Hl.
  (* Pick a witness to ensure the set is nonempty *)
  ...
  - (* min_local_period l r *)
    ...
    + (* Minimality: any w' with local_period satisfies w' >= r *)
      intros w' Hw'.
      ...
      * (* If w' ∈ [1, Zlength patn], use the nat minimality *)
        set (n' := Z.to_nat w').
        assert (HPn' : Pn n').
        { subst n'. split.
        ...
        lia.
      * (* If w' > Zlength patn: derive r <= Zlength patn from HnminP, then compare *)
...
(* Lemma: no full local period exists at the maximal-suffix cut *)
Lemma maxsuffix_cut_no_full_local_period (cmp_fn : A -> A -> comparison) (i : Z) (w : list A):
  ...
  w = nil.
Proof.
  intros Cmpfn. intros. (* Decompose x = u v, v = w t *)
  ...
  assert (Zsublist i (Zlength patn) patn = w ++ t). (* Use an append decomposition again *)
  { rewrite H9. subst t.
...
Proof.
  ...
  destruct H8; [|auto]. (* Derive a contradiction when i = 0 *)
...
(* Critical factorization from forward/reverse maximal-suffix algorithms *)
Theorem fwd_rev_maxsuffix_cut_critical_factorization:
  forall i j p,
    is_maximal_suffix patn cmp i -> 
    is_maximal_suffix patn cmp_rev j ->
    is_minimal_period default patn p -> (* Assume the global period is known *)
...
Proof.
  ...
  destruct H7.
  - (* First discuss the case i ≥ j (forward larger); overall symmetric *)
    replace (Z.max i j) with i by lia.
    assert (0 < i). (* First, derive u ≠ ε *)
    { apply (u_not_nil i j p); try auto.
      - split; try auto. (* These have been unpacked earlier in the proofs *)
      - split; try auto.
    ...
    { assert (r <= i \/ r > i) by lia.
      ...
      - set (w:= Zsublist i (i + r) patn).
        ...
        lia. (* Contradiction: r > 0 vs r = 0 from w = nil *)
      - (* Use properties related to maximal_suffix *)
    ...
    { rewrite Zlength_app. subst z. subst u.
      ...
      rewrite Zlength_Zsublist; try lia. }
    (* Continue with two cases: r > |v| or r <= |v| *)
    assert (r > Zlength patn - i \/ r <= Zlength patn - i) by lia.
    destruct H16.
    + (* r > |v| *)
    ...
    + set (z' := skipn' (i + r) patn).
      ...
      assert (r = p) by lia.
      rewrite <- H29. auto.
  - (* Symmetric for j: reuse i's proof with cmp <-> cmp_rev, i <-> j, u,v <-> u',v' *)
...
Proof.
  ...
  destruct H5.
  1:{ exists q. lia. } (* Exclude the case r > 0 *)
  ...
  { assert (Zsublist l (l + r) patn = Zsublist (l + w - r) (l + w) patn).
    ...
    reflexivity. }
  pose proof H2 r H6.
  lia. (* From local_period we derive r >= p, contradiction *)
Qed.

End critical_factorization_theorem.

Section match_algo_def.
(* Convention: pos is the alignment of text and patn (match position) *)
(* Convention: s is the length of the previously aligned prefix *)
(* Convention: j is the current comparison index during matching *)
...
Definition match_right_f (pos j: Z): program (CntOrBrk Z Z) :=
  choice
    ...
    (assume (j >= Zlength patn \/ ~match_cond pos j);; break j). (* Return the mismatch position *)

Definition match_right (pos start: Z): program Z :=
  repeat_break (match_right_f pos) start.

(* MARK: program structure closer to the paper *)
Definition match_left_f (pos s j: Z): program (CntOrBrk Z Z) :=
  choice
    (assume (j >= s /\ match_cond pos j);; continue (j - 1))
    (assume (j < s \/ ~match_cond pos j);; break j). (* On mismatch, directly return -1 (more natural) *)
    ...
    (assume (pos + Zlength patn <= Zlength text);;
      ...
      choice
        (assume (j = Zlength patn);;
          ...
          choice
            (assume (j' < 0);; break (Some pos)) (* Match successful *)
            (assume (j' >= 0);; continue (pos + p)) (* Left-side mismatch jump *)
        )
        (assume (j < Zlength patn);; continue (pos + j - l + 1)) (* Right-side mismatch jump *)
    )
    (assume (pos + Zlength patn > Zlength text);; break None). (* Failure: text too short *)

Definition match_algo (l p : Z) : program (option Z) :=
  repeat_break (loop_body l p) 0. (* Initialize pos = 0 *)
...
Record match_left_inv (l pos j: Z): Prop := {
  jrange_mli: - 1 <= j < l; (* Initial state *)
  Hpm_mli : Zsublist (j + 1) l patn = Zsublist (pos + j + 1) (pos + l) text
}.

(* Ideally no jrange needed *)
Record match_left_post (l pos j: Z): Prop := {
  Hjrange_mlp : -1 <= j < l;
  Hpm_mlp : Zsublist (j + 1) l patn = Zsublist (pos + j + 1) (pos + l) text;
  Hnpm_mlp: j < 0 \/ Znth j patn default <> Znth (pos + j) text default; (* j < s includes the initial non-satisfied state *)
}.
...
Proof.
  ...
  - intros j ?.
    ...
    reflexivity.
  - intros j ?.
    unfold match_left_f. (* Very concise! *)
    hoare_auto; destruct H0; destruct H1; destruct jrange_mli0;
    constructor; auto; try lia.
  - (* Final branch: prove the invariant holds initially *)
    constructor; [lia|].
    repeat rewrite Zsublist_nil; try lia.
    reflexivity.
Qed.
...
Proof.
  ...
  - unfold no_occur.
    ...
    destruct H1; [apply Hnoc0; lia|]. (* Only the jump step matters *)
    (* Prepare the first matching segment *)
    ...
    (* By contradiction, prepare the second matching segment *)
    intros Hnot.
    (* Discuss displacement t - pos vs. period p; multiples of p yield a contradiction *)
    set (q := (t - pos) / mp).
    set (r := (t - pos) mod mp).
    ...
    destruct H3.
    + (* Case r = 0: if displacement is a multiple of p, matching cannot reoccur *)
      assert (t - pos = mp * q) by lia.
      destruct Hnpm_mrp0; [lia|]. (* The first disjunct j >= Zlength patn is a range contradiction *)
      ...
      rewrite H8 in H7.
      contradiction.
    + (* Case r > 0: use text to match two copies of patn to obtain a local_period *)
      apply (f_equal (fun l' => Zsublist (l - (t - pos)) l l')) in Hnot.
      assert (local_period l (t - pos)).
      { assert (l - (t - pos) < 0 \/ 0 <= l - (t - pos)) by lia.
        destruct H5.
        - (* Original truncated subtraction -> actual insufficient-length issue *)
      ...
      (* Having obtained a local_period displacement, deduce t - pos = k * p and reach a contradiction *)
...
Proof.
  ...
  + intros Hnot.
    ...
    { unfold local_period.
      ...
      destruct H5; destruct H6. (* Discuss whether left/right lengths suffice *)
      ...
      - (* Left side insufficient length *)
        ...
        apply H8.
      - (* Right side insufficient length *)
    ...
    lia. (* Contradiction with the minimal local period *)
Qed.
...
Proof.
  ...
  intros j H3.
  hoare_auto. (* Right-side mismatch proof complete *)
  2:{ apply (match_inv_continue_right l p pos); auto; try lia. }
  apply Hoare_bind with (P := match_left_post l pos); subst j. (* Substitute j = Zlength patn from right-side match *)
  1:{ apply (match_left_prop l p pos); auto. } (* Left-side property uses part of right-side conclusion *)
...
Proof.
  ...
  destruct H1.
  - apply (Hnoc0 j H1).
  - intros Hnot. (* Refute using insufficient length *)
    ...
    rewrite Zlength_Zsublist in Hnot; try lia. (* Length mismatch contradiction *)
Qed.
...
Proof.
  intros.
  unfold loop_body.
  hoare_auto. (* Continue step by step *)
...
Proof.
  ...
  unfold match_algo. (* Enter match_algo *)
